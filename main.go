...
func startAPIServer() {
	...
	case http.MethodPost:
		...
		if err := json.NewDecoder(r.Body).Decode(&enabled); err != nil {
			http.Error(w, "Invalid JSON: expected raw true/false", http.StatusBadRequest)
			return
		}
	...
	case http.MethodGet:
		...
		// Return smaller JSON (frontend expects)
	...
	case http.MethodPost:
		...
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
			http.Error(w, "invalid JSON: expected { enabled: bool }", http.StatusBadRequest)
			return
		}
